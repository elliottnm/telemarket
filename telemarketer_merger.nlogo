turtles-own [ balance sales parent ]
...
to sell
  ...
  let maxcalls size * 100
  let target-customers potential-customers
  if maxcalls < count potential-customers [
  set target-customers n-of maxcalls potential-customers
  ]
  let success-sell target-customers with [ pcolor = green ]
  ask success-sell [ set pcolor red ]

  set sales count success-sell
end
...
to account
  ...
  if parent != nobody
  [
    ask parent [ set balance balance + ( 0.5 * profit ) ]
    set profit ( profit * 0.5 )
  ]
...
to find-merger
  let turtles-to-merge-with other turtles with [ size > [ size ] of myself and balance > ( -1 * [ balance ] of myself)]

  if any? turtles-to-merge-with
  [
    set parent one-of turtles-to-merge-with
    merge-with parent
  ]
end

to merge-with [ par ]
  ask par [ set balance balance + [ balance ] of myself ]
  set balance 0
end
@#$#@#$#@
GRAPHICS-WINDOW
